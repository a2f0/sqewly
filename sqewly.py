...
def main (argv):
    ...
    mysqlfilename = (server_address + "-" + strftime("%Y-%m-%d-%H%M",
        gmtime()) + ".mysql")

    gzipfilename = (server_address + "-" + strftime("%Y-%m-%d-%H%M",
        gmtime()) + ".tar.gz")
    ...
    backup_command = ("mysqldump --single-transaction -u " + database_username
        + " --password=" + database_password + " --databases --host " + 
        server_address + " " + database_name + " > " + outputfile)
...
def action_success():
    if (report_on_success == "1"):
        ...
        body = (body + "Most recent access: " + time.ctime(statinfo.st_atime)
            + "\n")
        body = (body + "Most recent modification: " 
